                ...
                @foreach($posts as $post)
                    <div class="col-md-6 col-sm-6">
                        <div class="blog-img-text res">
                            <div class="blog-img">
                                <a href="{{ route('blog.single', $post->slug) }}"><img src="{{ asset('images/posts/' . $post->image) }}" alt="" style="width: 300px; height: 260px;"></a>
                            </div>
                            <div class="blog-text">
                                <span>{{ date('M j, Y', strtotime($post->created_at)) }}</span>
                                <h4><a href="text-animator.html#">{{ $post->title }}</a></h4>
                                <p>{{ substr(strip_tags($post->body), 0, 80) }}{{ strlen(strip_tags($post->body)) > 80 ? "..." : "" }}</p>
                                <a href="{{ route('blog.single', $post->slug) }}">Read More <i class="ion-arrow-right-c"></i></a>
                            </div>
                        </div>
                    </div>
                @endforeach
